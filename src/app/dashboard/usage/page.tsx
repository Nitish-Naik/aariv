...
function getModelLabel(modelId: string): { label: string; hint: string } {
    ...
    if (id.includes("haiku") || id.includes("gpt-4o-mini") || id.includes("mini")) return { label: "Fast", hint: "Quick replies & summaries" };
    if (id.includes("opus") || id.includes("gpt-4-turbo") || id.includes("ultra")) return { label: "Ultra", hint: "Complex tasks & deep analysis" };
    if (id.includes("sonnet") || id.includes("gpt-4o") || id.includes("powerful")) return { label: "Powerful", hint: "Daily briefings & reasoning" };
    if (id.includes("gpt-3") || id.includes("standard")) return { label: "Standard", hint: "General assistant usage" };
...
function groupByDate(history: Transaction[]): { dateLabel: string; rows: Transaction[]; total: number }[] {
    const map = new Map<string, Transaction[]>();
    for (const tx of history) {
        const key = new Date(tx.date).toLocaleDateString("en-US", { year: "numeric", month: "short", day: "numeric" });
...
export default function UsagePage() {
    ...
    const [refillEnabled, setRefillEnabled] = useState(balanceData?.auto_refill_enabled ?? false);
