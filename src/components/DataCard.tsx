...
function EmailDataCard({ card }: { card: EmailCard }) {
  ...
  return (
    <div className="flex items-start gap-3 px-4 py-3 rounded-xl bg-black border border-white/10 hover:border-[var(--border-hover,rgba(255,255,255,0.1))] transition-colors">
      <div className="relative shrink-0">
        ...
        {card.isUnread && (
          <div className="absolute -top-0.5 -right-0.5 w-2.5 h-2.5 rounded-full bg-blue-400 border-2 border-[black]" />
        )}
      </div>
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between gap-2">
          <p className={`text-sm truncate ${card.isUnread ? "font-semibold text-white" : "font-medium text-neutral-400"}`}>
            {name}
          </p>
          <span className="text-[11px] text-neutral-500 shrink-0">{formatRelativeTime(card.date)}</span>
        </div>
        <p className="text-sm text-white truncate mt-0.5">{card.subject}</p>
        <p className="text-xs text-neutral-500 truncate mt-0.5">{card.snippet}</p>
      </div>
    </div>
  );
}
...
function CalendarDataCard({ card }: { card: CalendarCard }) {
  return (
    <div className="flex items-start gap-3 px-4 py-3 rounded-xl bg-black border border-white/10 hover:border-[var(--border-hover,rgba(255,255,255,0.1))] transition-colors">
      <div className="w-1 self-stretch rounded-full bg-white shrink-0" />
      <div className="flex-1 min-w-0">
        <p className="text-sm font-semibold text-white">{card.title}</p>
        <div className="flex items-center gap-1.5 mt-1">
          <Clock strokeWidth={1.5} size={12} className="text-neutral-500 shrink-0" />
          <span className="text-xs text-neutral-400">{formatTimeRange(card.startTime, card.endTime)}</span>
        </div>
        {card.location && (
          <div className="flex items-center gap-1.5 mt-1">
            <MapPin strokeWidth={1.5} size={12} className="text-neutral-500 shrink-0" />
            <span className="text-xs text-neutral-400 truncate">{card.location}</span>
          </div>
        )}
        {card.attendees && card.attendees.length > 0 && (
          <div className="flex items-center gap-1.5 mt-1">
            <Users strokeWidth={1.5} size={12} className="text-neutral-500 shrink-0" />
            <span className="text-xs text-neutral-500">{card.attendees.length} attendee{card.attendees.length > 1 ? "s" : ""}</span>
          </div>
        )}
      </div>
    </div>
...
function MessageDataCard({ card }: { card: MessageCard }) {
  return (
    <div className="flex items-start gap-3 px-4 py-3 rounded-xl bg-black border border-white/10 hover:border-[var(--border-hover,rgba(255,255,255,0.1))] transition-colors">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between gap-2">
          <p className="text-sm font-semibold text-white truncate">{card.sender}</p>
          <span className="text-[11px] text-neutral-500 shrink-0">{formatRelativeTime(card.timestamp)}</span>
        </div>
        <p className="text-sm text-neutral-400 mt-0.5 line-clamp-2">{card.content}</p>
        {card.channel && (
          <div className="flex items-center gap-1 mt-1">
            <Hash strokeWidth={1.5} size={11} className="text-neutral-500" />
            <span className="text-[11px] text-neutral-500">{card.channel}</span>
          </div>
        )}
      </div>
    </div>
