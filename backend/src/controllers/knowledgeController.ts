...
export const getKnowledgeGraph = async (req: Request, res: Response) => {
  try {
        ...
        nodes: [
          {
            ...
            label: "Connect Apps",
            description:
              "Connect Gmail or Calendar to generate a real knowledge graph.",
    ...
    REQUIRED ACTIONS:
    1. Fetch the last 5-10 email threads using 'GMAIL_LIST_THREADS' (lightweight) or 'GMAIL_FETCH_EMAILS'.
       - Focus on metadata (headers, subject, time) rather than full body to save tokens.
    2. Fetch the next 5 calendar events with 'GOOGLE_CALENDAR_LIST_EVENTS'.
    3. Analyze them for:
      ...
      if (msg.tool_calls) {
        ...
        for (let i = 0; i < msg.tool_calls.length; i++) {
          ...
          if (contentStr.length > 15000) {
            console.log(
              `[KnowledgeGraph] Truncating tool output from ${contentStr.length} to 15000 chars.`,
            );
            contentStr =
